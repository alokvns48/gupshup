...
const page = () => {
  ...
  const fetchAcceptMessage = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ApiResponse>("/api/accecpt-messeges");
  ...
  const handleSwitchChange = async () => {
    try {
      const response = await axios.post<ApiResponse>("/api/accecpt-messeges", {
  ...
  if (status === "loading") {
    ...
    return <Loader2 className="h-10 w-10 animate-spin m-auto" />;
  }
